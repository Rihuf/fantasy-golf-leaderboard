...
/*********************************************************
route:         GET /get_usopen_data
description:   Get tournament data and update golfer
               scores in my DB accordingly
**********************************************************/
router.get("/get_usopen_data", (request, response, next) => {
  axios
    .get("https://statdata.pgatour.com/r/026/leaderboard-v2mini.json")
    .then(res => {
      // console.log(res.data.leaderboard.players);
      const players = res.data.leaderboard.players;
      response.send("API hit success");
          ...
          Golfer.findOneAndUpdate(
            { golferId: index.player_id },
            {
              $set: {
                firstName,
                lastName,
                "tournaments.usopen.status": status,
                "tournaments.usopen.thru": thru,
                "tournaments.usopen.current_round": currentRound,
                "tournaments.usopen.today": today,
                "tournaments.usopen.scores.round1": round1,
